...
def load_functions(path: str) -> list[FunctionDef]:
    """Loads the functions JSON and validates if its valid based on the parameters
    i put in the models classes if it is return a list already validate and integrated in the
    FunctionDef class """
...
def load_tests(path: str) -> list[PromptItem]:
    """Loads the tests JSON and validates if its valid based on the parameters
    i put in the models classes if it is return a list already validate and integrated in the
    PromptItem class """
    with open(path, "r") as f:
        data = json.load(f)
...
def safe_test(path: str) -> list[PromptItem]:
    """ Makes the errors that can occur on the test files be much more easy to fix and doesnt crash the program 
...
def safe_definitions(path: str) -> list[FunctionDef]:
    """ Makes the errors that can occur on the definition files be much more easy to fix and doesnt crash the program 
    it closes it very gracefull"""
    try:
