...
def load_functions(path: str) -> list[FunctionDef]:
    ...
    with open(path, "r") as f:
        # json.load turns the json file into a dictionary of dictonaryes in
        # like prompt: dada value: 313 3131 3131
        data = json.load(f)
...
def safe_test(path: str) -> list[PromptItem]:
    ...
    try:
        ...
        print("The parameters of the input are not valid", e, file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError as e:
        print("JSON File Broken:", e, file=sys.stderr)
        sys.exit(1)
    except FileNotFoundError as e:
        print("JSON File Not found:", e, file=sys.stderr)
        sys.exit(1)
