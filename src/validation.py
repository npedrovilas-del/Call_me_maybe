...
from src.models import CallResult, FunctionDef
from src.generate import build_prompt, generate_call, to_ids
from src.vocab import get_token_text

def validate_result(obj: dict[str, object], functions: list[FunctionDef]) -> CallResult:
    ...
    if set(params) != set(func.parameters):
        raise ValueError(f"parameter keys do not match {func.name}: {set(params)}")
    for key, param in func.parameters.items():
        value = params[key]
        if param.type == "number":
            if not isinstance(value, (int, float)) or isinstance(value, bool):
                raise ValueError(f"{key}: number expects int|float, got {type(value).__name__}")
        elif param.type == "string":
            if not isinstance(value, str):
                raise ValueError(f"{key}: string expects str, got {type(value).__name__}")
        elif param.type == "boolean":
            if not isinstance(value, bool):
                raise ValueError(f"{key}: boolean expects bool, got {type(value).__name__}")
    return CallResult(prompt=str(obj["prompt"]), name=func.name, parameters=params)


def run(model, functions, tests) -> list[CallResult]:
    ...
    for item in tests:
        try:
            ...
            obj = generate_call(model, base_ids, item.prompt, functions, token_text)
